...
public class ProtocolManager : MonoSingleton<ProtocolManager>
{
    ...
    public delegate void WebControllerEvent(ProtocolType protocolType, string connID);
    ...
    private void ReceiveMessage(string conn_id, string key_code, string color_id)
    {
        TraceBox.Log("Message Received / connId: " + conn_id + " / key_code: " + key_code + " / value: " + color_id);
        switch (key_code)
        {
            case "game_replay":
                {
    ...
    private void OnUserExit(PlayerData playerData)
    {
        ...
        else if (JoyStreamCommunicator.instance.GetPlayerCount() == 50)
        {
    ...
    public void OnReceivedControllerFall_Press(string conID) // 4.9 추가
    {
